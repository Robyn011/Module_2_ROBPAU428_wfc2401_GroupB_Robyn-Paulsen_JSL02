...
const addNewGoal = () => {
    ...
    if (goalInput !== '') {
        ...
        if (Duplicate) {
            // Display alert for the duplicated goal
            alert('Goal already exists. Please enter a new goal');
        } else {
            // If it's not a duplicate, then proceed by adding it as a new goal
            const newGoal = document.createElement('li');
            newGoal.textContent = goalInput;
            goalList.appendChild(newGoal);
        }
    } else {
        // Display an alert for an empty input
        alert('No goal was entered. Please input a valid goal.');
    }
};
